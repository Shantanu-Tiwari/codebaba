...
export async function disconnectRepository(repositoryId:string){
    try {
        ...
        await deleteWebhook(await headers(), repository.owner, repository.name)
...
export async function disconnectAllRepositories() {
    try {
        ...
        // Delete webhooks for all repositories
        for (const repo of repositories) {
            try {
                await deleteWebhook(await headers(), repo.owner, repo.name);
            } catch (error) {
                console.error(`Failed to delete webhook for ${repo.owner}/${repo.name}:`, error);
            }
        }
